...
-----------------------------
1. PB1 (DI8 / PLC X102) -> Palletize (1 Loop, max 8 items):
...
2. PB2 (DI9 / PLC X103) -> Smart LIFO Put-Back (De-palletize):
...
from neuromeka import IndyDCP3, OpState, TaskBaseType
...
DI_PB1             = 8   # PLC X102 -> Push Button 1 (Start Palletize Loop)
DI_PB2             = 9   # PLC X103 -> Push Button 2 (Put Back to Feeder)
...
def run_palletize_sequence(indy: IndyDCP3, current_count: int) -> int:
    ...
    Breaks immediately if DI3 (Magazine Sensor) is OFF.
    ...
    for i in range(current_count, TOTAL_MAX_ITEMS):
        # 1. Check Magazine Sensor (DI3) -> Break if OFF
        ...
        drop_approach = get_approach_pose(slot_pose)
...
def run_put_back_sequence(indy: IndyDCP3, current_count: int) -> int:
    ...
    for i in range(current_count - 1, -1, -1):
        slot_pose = get_pallet_slot_pose(i)
        slot_approach = get_approach_pose(slot_pose)
        floor = i // SLOTS_PER_FLOOR
...
def run_plc_listener(indy: IndyDCP3):
    ...
    print("=" * 65)
    print(f"  * PB1 (PLC X102 -> DI{DI_PB1}): Palletize Loop (Max 8, breaks if DI3=OFF)")
    print(f"  * PB2 (PLC X103 -> DI{DI_PB2}): Smart Put-Back (LIFO -> Top of Feeder)")
    ...
    while True:
        try:
            ...
            if now - last_status_time > 1.5:
                ...
                print(f"\r[STATUS] DI3(Mag): {status_mag} | PB1: {status_pb1} | PB2: {status_pb2} | Pallet Items: [{pallet_count}/8]", end="", flush=True)
